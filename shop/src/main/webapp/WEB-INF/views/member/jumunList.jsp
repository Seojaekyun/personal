			...
			<c:if test="${chk == 0}">
				<div class="onePro" id="left">
					...
					<div id="pro4">
						...
						<c:if test="${(mapAll.get(i).state==3||mapAll.get(i).state==8) && mapAll.get(i).isReview==0}">
							<input type="button" value="리뷰쓰기" id="btn" onclick="location='reviewWrite?id=${mapAll.get(i).id}&pcode=${mapAll.get(i).pcode}'">
						</c:if>
						<c:if test="${(mapAll.get(i).state==3||mapAll.get(i).state==8) && mapAll.get(i).isReview==1}">
