...
public class MemberServiceImpl implements MemberService{
	...
	public String cartView(HttpSession session, HttpServletRequest request, Model model) {
		...
		if(session.getAttribute("userid")==null) { // 쿠키변수에서 읽은 후 상품코드를 분리하여 가져온다
			Cookie cookie=WebUtils.getCookie(request, "pcode");
			if(cookie!=null&&!cookie.getValue().equals("")) {
				String[] pcodes=cookie.getValue().split("/");
				
				pMapAll=new ArrayList<HashMap>(); // for(int i=0;i<pcodes.length;i++) // 모든 요소를 다 실행해야 된다.
		...
		if(pMapAll!=null) {
			...
			for(int i=0;i<pMapAll.size();i++) {
				...
				int jukprice=(int)(halinprice*juk/100.0)*su; // 하나의 상품구매 적립금
	...
	public int[] chgSu(HttpServletRequest request, HttpSession session, HttpServletResponse response) {
		...
		tot[1]=(int)((price-(price*halin/100.0))*juk/100.0)*su;
	...
	public String jumunList(HttpSession session, Model model, HttpServletRequest request) {
		...
		for(int i=0;i<mapAll.size();i++) {
			if(imsi.equals(mapAll.get(i).get("jumuncode").toString())) {
				mapAll.get(i).get("title"); mapAll.get(i).get("pimg");
				mapAll.get(i).get("name"); mapAll.get(i).get("juso");
				mapAll.get(i).get("chongPrice"); mapAll.get(i).get("state");
				mapAll.get(i).get("su"); mapAll.get(i).get("useJuk");
				mapAll.get(i).get("sudan"); mapAll.get(i).get("writeday");
			}
			else {
	...
	/*
	@Override public String jumunList2(HttpSession session, Model model) {
		String userid=session.getAttribute("userid").toString(); ArrayList<HashMap>
		mapAll=mapper.jumunList(userid);
		
		for(int i=0;i<mapAll.size();i++) { // state의 값을 문자열로 변경후 저장 String
			state=mapAll.get(i).get("state").toString(); String stateMsg=null;
			switch(state) {
				case "0": stateMsg="결제완료"; break;
				case "1": stateMsg="상품준비중"; break;
				case "2": stateMsg="배송중"; break;
		...
		model.addAttribute("mapAll",mapAll); return "/member/jumunList2";
	}
	
	@Override public String jumunList3(HttpSession session, Model model) {
		String userid=session.getAttribute("userid").toString();
		List<HashMap> mapAll=mapper.jumunList(userid);
		
		for(int i=0;i<mapAll.size();i++) { // state의 값을 문자열로 변경후 저장
			String state=mapAll.get(i).get("state").toString(); String stateMsg=null;
			switch(state) {
				case "0": stateMsg="결제완료"; break;
				case "1": stateMsg="상품준비중"; break;
				case "2": stateMsg="배송중"; break;
		...
		model.addAttribute("mapAll",mapAll); return "/member/jumunList3";
	}
	*/
